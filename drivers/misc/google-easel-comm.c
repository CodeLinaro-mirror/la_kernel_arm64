...
static struct easelcomm_service *easelcomm_service_list[EASELCOMM_SERVICE_COUNT];
	...
	struct easelcomm_kmsg_desc __user *pmsg_desc)
{
	...
	if (msg_desc->message_size > EASELCOMM_MAX_MESSAGE_SIZE) {
		dev_err(easelcomm_miscdev.this_device,
			"%s: svc:%u msg size %u is larger than max size %d\n",
			__func__,
			service->service_id,
	...
	unsigned long arg, bool compat)
{
	...
	if (compat) {
		...
		if (copy_from_user(&compat_kbuf_desc, (void __user *) arg,
					sizeof(compat_kbuf_desc)))
			return -EFAULT;
		...
		if (copy_from_user(&kbuf_desc, (void __user *) arg,
					_IOC_SIZE(cmd)))
			return -EFAULT;
	}
